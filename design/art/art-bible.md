...
인게임 동적 액터 레이어(풍선·캐릭터·작살 비드·UI 패널·아이템 아이콘)는 파편화된 리소스가 아닌 단일한 글래스 사양 구조로 바인딩됨. **배경 레이어(Frosted Sky 그라데이션 + Frost Cloud Overlay, §6.2)는 본 원칙 적용 제외** — 배경은 flat gradient + 절차적 frost cloud overlay (§6.2)이며 글래스 머티리얼 아님. "Frosted" 명칭의 시각 실체는 단순 그라데이션이 아닌 안개 느낌 부여.
...
AI 에이전트는 그래픽 렌더러 컴포넌트 (visual-juice + entity-textures + background 모듈) 구현 시 해당 HTML 샘플 코드에 선언된 파라미터 값(Opacity, Blur 12px, 색상 hex 등)을 기본값으로 상속받아 정밀 렌더링해야 함.
...
#### §S3 Critical Moment — 사용자 인지 cue 통합 표

Critical Pop 0.2s 시퀀스 중 사용자가 동시에 경험하는 시각 cue 전체:

| # | 효과 | 지속 | 정의 위치 |
|---|------|------|----------|
| 1 | **Critical Gold 풍선 자체 differentiation** — texture gold + scale ×1.1 + hero GlowFilter + hot-spec | spawn 시점부터 상시 | **본 §4.2 Critical Gold 사양** |
| 2 | **배경 cool blue overlay** — vfxContainer alpha 0 → 0.6 → 0 (color `0x0a1f3a`) 5-phase tween | 0.20s | visual-juice GDD §3.3 |
| 3 | **캐릭터 화이트-핫 GlowFilter swap** — `{ distance: 30, outerStrength: 2.5, color: 0xFFFFFF }` | 0.20s + 원상복구 | visual-juice GDD §3.x |
| 4 | **White flash overlay** — full-screen alpha 0→1→0 | 0.05s | visual-juice GDD §3.6 |
| 5 | **Glass shard particle burst** — sharp 3-4 vertex 다각형 (4 종류 랜덤) + 4-layer Layered Translucency (frosted base alpha 0.45 / inner 0.65 / rim 0.95 / specular 1.0) + tumbling ±8 rad/s | ~1.2s | visual-juice GDD §3.2 |

> **관할 경계**: cue 1은 art-bible + balloon-physics-split 관할. cue 2–5는 visual-juice GDD + VisualJuiceSystem 관할. art-director는 cue 1의 hex·strength·scale 수치를 소유하고, cue 2–5의 수치 변경은 visual-juice GDD 개정 후 여기 cross-ref만 갱신.
...
|--------|------------------|--------|------|
| **XL** | **160px** | → Large 2개 | 2.0 (최대 시작 크기) |
| Large | 80px | → Medium 2개 | 1.0 |
| Medium | 56px | → Small 2개 | 0.7 |
| Small | 38px | → XS 2개 | 0.48 |
| **XS** | **19px** | (종단 — 안 쪼개짐) | 0.24 (분열 종단 크기) |
...
#### Critical Gold 풍선 spawn 시각 사양

`applyCriticalVisual(b)` API를 balloon-physics-split System이 노출, critical-pop System이 `_setCritical()` 호출 시점에 호출 — 3-effect (texture swap + scale + glow) 동시 적용. **구현 권위**: entity-textures 모듈 (texture) + balloon-physics-split System (scale + filter).

| 항목 | 사양 | sample HTML 정합 |
|------|------|----------------|
| **Texture** | `getBalloonTexture(app, 'gold')` — Canvas 2D `createRadialGradient` (center 35% 30% bias). BALLOON_PALETTE.gold: center `#FFF0AA`, mid `#FFD700`, edge `#C89C00` | sample L114: `radial-gradient(circle at 35% 30%, #FFF0AA 0%, #FFD700 50%, #C89C00 100%)` — 정확 일치 |
| **Tint** | `0xFFFFFF` (no multiply) — texture 자체가 gold gradient이므로 colorId tint 무적용. 일반 풍선도 동일. | — |
| **Scale** | `sprite.width = BALLOON_BASE_DIAMETER × SIZE_RATIO[size] × 1.1` (height 동일) — §S3 "scale ×1.1" 정확 구현 | — |
| **GlowFilter** | `{ distance: 28, outerStrength: 2.0, innerStrength: 0, color: 0xFFD700, quality: 0.5 }` — §6 heroGlow 정합. supporting 0.8의 2.5× | sample L115-117: `box-shadow: 0 0 48px rgba(255,215,0,0.8), 0 0 16px rgba(255,240,170,1.0)` — 강도 정합 |
| **Extra hot-spec** | Gold 분기 추가: `blur(3px)`, radius `size×0.17`, `rgba(255,255,220,0.70)` white-yellow specular (step 5 in `_makeBalloonCanvas`) | sample L122-125: `.b-gold::before { background: rgba(255,255,220,0.7); width: 35%; height: 35%; }` — 정확 일치 |
| **Outline stroke** | `#FFD700` 2px — S3 다크닝 진입 시만 (§3.3 Critical Gold 풍선 silhouette 변화). texture layer 아님 — Pixi Graphics overlay | sample L116 inner glow inset 대응 |

> **texture 소유**: entity-textures.getBalloonTexture('gold') — technical-artist 단독. balloon-physics-split 또는 critical-pop에서 직접 수정 금지.
...
// 일반 풍선(magenta/cyan/lime/amber/mint/violet) — supporting tier 적용 의무.
// glow color = BALLOON_PALETTE[colorId].mid (각 풍선 색에 따른 네온 halo).
// quality: 0.3 (모바일 60fps 유지 — GlowFilter 모바일 perf 최소화).
const supportingGlow = new GlowFilter({ distance: 12, outerStrength: 0.8, innerStrength: 0, color: 0xFF3DA5 /* per-color */, quality: 0.3 });
// Critical Gold + Power-Up — hero tier (visual-juice가 swap)
const heroGlow = new GlowFilter({ distance: 28, outerStrength: 0.85, innerStrength: 0, color: 0xFFD700, quality: 0.5 });

// 3. 배경 그라데이션 + Frost Cloud Overlay — §6.2 절차적 생성 참조
// Pre-rendered PNG 미사용 — 절차적 생성으로 < 20KB, 0KB 런타임 메모리.

// 4. Balloon Texture — Canvas 2D radial gradient (§4.2 Critical Gold 사양)
// Pixi Graphics는 radial gradient 미지원 → Canvas 2D `createRadialGradient` + blur filter
// + clip + shadow workaround로 샘플 HTML CSS `radial-gradient(circle at 35% 30%, ...)`
// + `::before` blur highlight + `::after` inset rim glow 정밀 이식. `Texture.from(canvas)` 변환.
...
```typescript
// Pixi v8 절차적 Frosted Sky gradient + Frost Cloud Overlay 생성
...
g.rect(0, 0, screen.width, screen.height).fill({ fill: gradient });

// Frost cloud overlay — 8 흰 ellipse 절차적 흩뿌림.
// 고정 seed (12345) → 매 빌드 같은 layout. 같은 texture에 베이킹 → render overhead 0.
let seed = 12345;
const nextRng = () => { seed = (seed * 1103515245 + 12345) & 0x7fffffff; return seed / 0x7fffffff; };
for (let i = 0; i < 8; i++) {
  const cx = nextRng() * screen.width;
  const cy = nextRng() * screen.height;
  const rx = 80 + nextRng() * 200;  // 80–280 px
  const ry = 40 + nextRng() * 80;   // 40–120 px
  g.ellipse(cx, cy, rx, ry).fill({ color: 0xffffff, alpha: 0.18 });
}
// generateTexture → Sprite로 베이킹 (런타임 draw call 1회)
```

**Frost Cloud Overlay 파라미터 lock**:

| 항목 | 값 | 의미 |
|------|----|----- |
| 클라우드 개수 | 8 | 너무 많으면 시각 산만, 적으면 frost 느낌 약함 |
| alpha | 0.18 | 너무 진하면 그라데이션 가림, 0.18은 "안개" 느낌 sweet spot |
| 크기 | rx 80–280px, ry 40–120px | 가로로 긴 cloud (수평 layered atmosphere) |
| seed | 12345 (고정) | 같은 layout 재현 — runtime random 시 빌드마다 다른 frost는 노이즈 |
| 동적 drift | 없음 (M0) | drift 애니메이션은 polish (M1 retrofit) |
...
| SFX 7개 (PowerShell 합성 6개 + freesound UI 1개) | ~30–40KB | 60KB |
| 캐릭터 PNG | ~5KB | 10KB |
